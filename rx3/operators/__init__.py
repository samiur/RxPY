...
def buffer_when(closing_mapper: Callable[[], Observable]) -> Callable[[Observable], Observable]:
    ...
    Examples:
        >>> res = buffer_when(lambda: rx3.timer(0.5))
...
def buffer_toggle(openings: Observable,
    ...
    >>> res = buffer_toggle(rx3.interval(0.5), lambda i: rx3.timer(i))
...
def delay_with_mapper(subscription_delay=None,
    ...
    Examples:
        ...
        >>> res = source.delay_with_mapper(rx3.timer(2.0), lambda x: rx3.timer(x))
...
def group_by_until(key_mapper: Mapper,
    ...
    Examples:
        >>> group_by_until(lambda x: x.id, None, lambda : rx3.never())
        >>> group_by_until(lambda x: x.id, lambda x: x.name, lambda grp: rx3.never())
...
def sequence_equal(second: Observable, comparer: Optional[Comparer] = None
    ...
    Examples:
        ...
        >>> res = sequence_equal(rx3.return_value(42))
        >>> res = sequence_equal(rx3.return_value({ "value": 42 }), lambda x, y: x.value == y.value)
...
def slice(start: Optional[int] = None,
    ...
    :func:`skip <rx3.operators.skip>`,
    :func:`skip_last <rx3.operators.skip_last>`,
    :func:`take <rx3.operators.take>`,
    :func:`take_last <rx3.operators.take_last>` and
    :func:`filter <rx3.operators.filter>`.
...
def throttle_with_mapper(throttle_duration_mapper: Callable[[Any], Observable]) -> Callable[[Observable], Observable]:
    ...
    Example:
        >>> op = throttle_with_mapper(lambda x: rx3.Scheduler.timer(x+x))
...
def timeout_with_mapper(first_timeout: Optional[Observable] = None,
    ...
    Examples:
        >>> res = timeout_with_mapper(rx3.timer(0.5))
        >>> res = timeout_with_mapper(rx3.timer(0.5), lambda x: rx3.timer(0.2))
        >>> res = timeout_with_mapper(rx3.timer(0.5), lambda x: rx3.timer(0.2)), rx3.return_value(42))

    Args:
        first_timeout: [Optional] Observable sequence that represents
            the timeout for the first element. If not provided, this
            defaults to rx3.never().
        ...
        other: [Optional] Sequence to return in case of a timeout. If
            not provided, this is set to rx3.throw().
...
def window_when(closing_mapper: Callable[[], Observable]) -> Callable[[Observable], Observable]:
    ...
    Examples:
        >>> res = window(lambda: rx3.timer(0.5))
...
def window_toggle(openings: Observable,
    ...
    >>> res = window(rx3.interval(0.5), lambda i: rx3.timer(i))
